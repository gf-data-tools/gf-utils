...
class TextTable(MutableMapping):
    ...
    def __get_text_table(self, table_file):
        ...
        for lineno, line in enumerate(
            (self.table_dir / f"{table_file}.txt").open("r", errors="ignore")
        ):
            try:
    ...
    def __setitem__(self, key, value):
        raise TypeError("TextTable does not support manual setting")

    def __delitem__(self, key):
        raise TypeError("TextTable does not support manual deletion")
