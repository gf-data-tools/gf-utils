...
class BattleSkillConfig(ConfigTable):
    ...
    def add_instance(self,k):
        ...
        for i in range(k*100+1,k*100+11):
            ...
            for key, value in self._data[i].items():
                if key=='action_id':
                    value = self.gamedata.battle_action_config[self.gamedata.get_value(value)]
                elif key=='target_select_ai':
                    value = self.gamedata.battle_target_select_ai[self.gamedata.get_value(value)]
                    ...
                    if not value:
                        ...
                        value = self.gamedata.get_value(value)
                    value = [SkinAction(skin=i,action=self.gamedata.battle_action_config[v]) for i, v in value]
                elif key=='trigger_id':
                    if not value:
                        value = []
                    elif ',' not in value:
    ...
    def __iter__(self): return iter({i['skill_group_id'] for i in self._data.values()})
    def __len__(self): return len({i['skill_group_id'] for i in self._data.values()})